$(window).on("load", function () {
    ...
    $("#droppable").droppable({
        drop: function (event, ui) {
            ...
            $.getJSON(url, function (data) {
                ...
                // Si réponse ok, la zone droppable garde sa couleur initiale
                ...
                for (var i = 0; i < 4; i++) {
                    if (data.quizz[niveau][drag.attr("id")].propositions[i] === data.quizz[niveau][drag.attr("id")].réponse) {
                        //$("li[ident=" + i + "]").css("background", "rgb(53, 179, 28)");
    ...
    $("input[type=radio][name=niveau]").on("change", function () {
        ...
        $("#suivant").show();
        $("#suivant").text("Démarrez le Quizz");
        $("#container").hide();
        $("#quizz").empty();
